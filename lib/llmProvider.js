...
Do NOT include markdown code fences (\`\`\`json), explanations, or intro text. Output raw JSON array only.`;
  ...
  if (isLocalMode) {
    ...
    for (const modelName of modelsToTry) {
      const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${cleanApiKey}`;
      try {
        ...
        if (response.status !== 404) {
          throw new Error(`Gemini LLM API returned HTTP ${response.status}: ${lastErrText.substring(0, 200)}`);
        }
      } catch (netErr) {
        if (netErr.message.includes('HTTP')) throw netErr;
        throw new Error(`Failed to reach Gemini LLM API: ${netErr.message}`);
      }
    }

    if (!response || !response.ok) {
      throw new Error(`Gemini LLM API error: ${lastErrText.substring(0, 200) || 'Unknown error'}`);
    }
